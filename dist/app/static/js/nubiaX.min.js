...
var glass = function () {
  return {
    init: function init(x) {
      ...
      this.$main = document.querySelector("#main");
      this.$main_left = document.querySelector(".main_left"); // 获取每一张图片的li集合
    ...
    event: function event() {
      ...
      this.$showImage.onmousemove = function (ev) {
        ...
        var y = ev.pageY - _this.$filter.offsetHeight / 2 - 115; // 获取小方块移动的最大坐标
...
var pushData = function () {
  ...
  return {
    ...
    getData: function getData() {
      ...
      sendAjax('shoplist.json').then(function (res) {
        res = JSON.parse(res); //获取相对应的json对象数据
        //先获取当前选择的颜色款

        dataNum = [];
        var colorType = $('#selector').children().children(".select").html();
        ...
        console.log(dataNum); // this.data=res;

        _this2.insertData(dataNum);
      });
    },
    insertData: function insertData(data) {
      //先获取当前登录名
      var user = $(".loginname").html(); //获取本地存储的key值为user的数据，如果没有就创建空数组字符串

      var shoplist = localStorage.getItem($(".loginname").html()) || "[]"; //将json字符串转化对象

      shoplist = JSON.parse(shoplist);
      console.log(shoplist);
      console.log(data); //把新数据添加，同时判断新添加的数据是否已经添加
